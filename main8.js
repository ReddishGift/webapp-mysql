...
const db = mysql.createConnection({  //DB와 서버 연결 객체 생성
...
app.get('/', (req, res) => {
    ...
    db.query(`select * from teacher`, (err, teacher) => {
        ...
        if (name === undefined) {
            ...
            db.query(`select teacher.id as teacher_id, name, subject, class, room from teacher join office on office_no=office.id where teacher.id=?`, [name], (err2, teacher2) => {
                if (err2) throw err2;
                name = teacher2[0].name
                data = `과목 : ${teacher2[0].subject}, 학급 : ${teacher2[0].class}, 교무실 : ${teacher2[0].room}`
                control = `<a href="/create">create</a> <a href="/update?name=${teacher2[0].teacher.id}">update</a>
                <form action="delete_process" method="post">  <!-- 링크로 이동하지 않고 바로 삭제 구현 -->
                    <input type='hidden' name='id' value='${teacher2[0].teacher_id}'>
                    <button type="submit">delete</button>
                </form>
                `
                const html = template.HTML(name, lilst, `<h2>${name} 페이지</h2><p>${data}</p>`, control)
                res.send(HTML)
            })
        }
    })
...
app.post('/create_process', (req, res) => {
    ...
    req.on('end', () => {
        ...
        db.query(`insert into teacher (name, subject, created, class, office_no) values (?, ?, now(), ?, ?)`, [title, description, null, 1], (err, result) => {
            if (err) throw err;
